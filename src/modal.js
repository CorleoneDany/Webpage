...
export default async function setModal() {
    ...
    for (let film of films) {
        const newFilm = film.cloneNode(true);
        film.parentNode.replaceChild(newFilm, film);

        newFilm.addEventListener("click", async function (event) {
            let filmInfo = await getFilmDetail(newFilm.getAttribute("data-url"))
